  ...
  ({ resume }, ref) => {
    ...
    return (
      ...
      >
        ...
        {/* Fallbacks for templates not implemented yet */}
        {resume.templateId === 'executive' && <ClassicATS resume={resume} />}
        {resume.templateId === 'startup' && <ModernProfessional resume={resume} />}
      </div>
    );
  }
);
...
const ClassicATS = ({ resume }: { resume: Resume }) => (
  <div className="p-10 text-black font-serif">
    ...
    <header className="text-center mb-6 border-b-2 border-black pb-4">
      ...
      {resume.targetRole && (
        <div className="text-lg font-semibold uppercase tracking-widest text-gray-700 mb-3">
          {resume.targetRole}
        </div>
      )}
...
const ModernProfessional = ({ resume }: { resume: Resume }) => (
  <div className="p-10 text-gray-900 h-full">
    {/* Header */}
    <header className="mb-8 flex flex-col items-start border-l-4 pl-4" style={{ borderColor: 'var(--theme-primary)' }}>
      ...
      <div className="font-medium text-lg mb-3 tracking-wide" style={{ color: 'var(--theme-primary)' }}>{resume.targetRole}</div>
            ...
            <div key={exp.id} className="break-inside-avoid">
              <div className="flex justify-between items-start mb-1">
                <div>
                  ...
                  <div className="text-[14px] font-semibold" style={{ color: 'var(--theme-primary)' }}>{exp.company} <span className="text-gray-500 font-normal ml-1">• {exp.location}</span></div>
                </div>
                <div className="text-xs font-bold px-2 py-1 rounded" style={{ backgroundColor: 'color-mix(in srgb, var(--theme-primary) 10%, transparent)', color: 'var(--theme-primary)' }}>
...
const ProductDesigner = ({ resume }: { resume: Resume }) => (
  <div className="flex h-full min-h-[1131px] text-slate-800">
    {/* Left Sidebar */}
    <aside className="w-[30%] bg-slate-900 text-slate-100 p-8 flex flex-col break-inside-avoid" style={{ backgroundColor: 'var(--theme-primary)' }}>
      <div className="mb-10">
        ...
        <div className="font-medium tracking-wide text-sm opacity-90">{resume.targetRole}</div>
      </div>
