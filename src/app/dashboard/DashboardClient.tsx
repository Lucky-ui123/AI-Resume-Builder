...
import {
  FileText, Target, TrendingUp, Briefcase, Clock, History, Loader2, LayoutDashboard
} from 'lucide-react';
    ...
    <div className="flex h-full flex-col overflow-y-auto bg-muted/10 font-sans">
      <div className="sticky top-0 z-20 px-4 md:px-6 bg-card border-b border-border/40 shrink-0">
        <PageHeader
          ...
          description="Here is what is happening with your job search today."
          actions={
        ...
        <PageContent>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            <StatsCard
              ...
              value={
                <div>
                  <div>{profileStrength}%</div>
                  <Progress value={profileStrength} className="mt-3 h-1.5 w-full" />
                </div>
              }
              subtitle="Based on your active resumes"
            />
            <StatsCard
              icon={<Target />}
              title="Avg ATS Score"
              value={`${stats.latestAtsScore}/100`}
              subtitle={atsFeedback}
            />
            <StatsCard
              icon={<FileText />}
              title="Resumes Created"
              value={resumes.length}
              subtitle="Total created so far"
            />
            <StatsCard
              icon={<Briefcase />}
              title="Cover Letters"
              value={stats.coverLetters}
              subtitle="Total generated so far"
            />
          </div>
