...
import { Lock, FileText, Plus, Sparkles } from 'lucide-react';
import { Resume } from '@/types';

export default async function AiSuggestionsPage({ searchParams }: { searchParams: { id?: string } }) {
      ...
      <div className="p-4 md:p-8 w-full space-y-8 font-sans">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight">AI Resume Suggestions</h1>
              <p className="text-muted-foreground mt-1 text-lg">Get intelligent recommendations to improve your resume content.</p>
            </div>
          </div>
        </div>
          ...
          <Card className="border-border shadow-sm">
            <CardHeader className="text-center pb-2">
              ...
              <CardDescription className="text-base mt-2">
                AI Suggestions are available on our Pro and Premium plans.
              </CardDescription>
            </CardHeader>
            <CardContent className="text-center text-muted-foreground pt-4 pb-8">
              <p>Upgrade your account to get line-by-line AI recommendations for improving impact, action verbs, and quantifying your achievements.</p>
            </CardContent>
  ...
  if (id) {
    ...
    return <AiSuggestionsClient resume={resume} />;
  }
  ...
  if (resumes.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center px-4">
        ...
        <p className="text-muted-foreground mb-6 max-w-md">You need to create a resume first before you can get AI suggestions.</p>
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight mb-2">Select a Resume for AI Suggestions</h1>
        <p className="text-muted-foreground">Choose which resume you want our AI to analyze and improve.</p>
      </div>
      <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
        {resumes.map((r: Resume) => (
          <Link href={`/dashboard/ai-suggestions?id=${r.id}`} key={r.id} className="block group">
